...
public class CourseMapTest
{
	...
	public void addLectureBeforeCourseTest()
	{
		...
		List<CourseLecturePair> course1Pairs = courseMap.getCourseMap().get(courseName1);
		...
		Assert.assertEquals("Lecture name was not expected", lectureName1, course1Pairs.get(0).getLecture().getLectureName());
		Assert.assertEquals("Lecture should be initialized to 0 students", 0, course1Pairs.get(0).getLecture().getClassSize());
	}
	...
	public void addMultipleCoursesAndLecturesTest()
	{
		...
		List<CourseLecturePair> course1Pairs = courseMap.getCourseMap().get(courseName1);
		List<CourseLecturePair> course2Pairs = courseMap.getCourseMap().get(courseName2);
		
		Assert.assertEquals("Object should be only one in course map", 2, course1Pairs.size());
		
		Assert.assertEquals("Lecture name was not expected", lectureName1, course1Pairs.get(0).getLecture().getLectureName());
		Assert.assertEquals("Lecture name was not expected", lectureName2, course1Pairs.get(1).getLecture().getLectureName());
		
		Assert.assertEquals("Course name was not expected", courseName1, course1Pairs.get(0).getCourseName());
		Assert.assertEquals("Course name was not expected", courseName2, course2Pairs.get(0).getCourseName());
		
		Assert.assertEquals("Lecture name was not expected", lectureName1, course2Pairs.get(0).getLecture().getLectureName());
		Assert.assertEquals("Lecture name was not expected", lectureName2, course2Pairs.get(1).getLecture().getLectureName());
		
		Assert.assertEquals("Lecture should be initialized to 0 students", 0, course1Pairs.get(0).getLecture().getClassSize());
		Assert.assertEquals("Lecture should be initialized to 0 students", 0, course1Pairs.get(1).getLecture().getClassSize());
		
		Assert.assertEquals("Lecture should be initialized to 0 students", 0, course2Pairs.get(0).getLecture().getClassSize());
		Assert.assertEquals("Lecture should be initialized to 0 students", 0, course2Pairs.get(1).getLecture().getClassSize());
	}
}
